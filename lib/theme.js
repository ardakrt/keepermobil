    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        supabase
          ...
          .select('theme_mode_mobile, accent_color')
          ...
          .then(({ data, error }) => {
            if (error) {
              ...
              if (data.theme_mode_mobile) {
                setMode(data.theme_mode_mobile);
                AsyncStorage.setItem(STORAGE_KEY, data.theme_mode_mobile);
              }
